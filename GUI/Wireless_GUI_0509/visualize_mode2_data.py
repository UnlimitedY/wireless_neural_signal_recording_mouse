...
# 文件路径
# AP + LFP for test
file_path = r"E:\wirelessNeuralRecording-mouse\firmware\LFP\QtESBV3\Wireless_GUI_0509\test_data_ner\motion_noise_test\mode2\Battery2025-05-29-16-37-17mode2.npy"
...
def visualize_lfp_data(data, fs=1000):
    """可视化 LFP 数据"""
    original_fs = fs # 保存原始采样率，假设输入是20kHz
    downsample_factor = original_fs // 1000 # 计算下采样因子
    new_fs = original_fs // downsample_factor # 新的采样率，应为1kHz
    ...
    if "TimeStamp" in data and len(data["TimeStamp"]) > 0:
        ...
        for i in range(len(original_time_stamps)): # 
            ...
            if(i + 1 < len(original_time_stamps)):
                ...
                if(next_time - current_time) < 3: # 没有丢包
        ...
        print("loss_num:", loss_packets_num)
    ...
    # 将通道数据转换为数组并转置以便绘制热图
    channel_data_np = np.array(channels)
    
    # 创建热图
    sns.heatmap(channel_data_np, cmap="viridis", xticklabels=False, yticklabels=False, cbar=False)
    
    plt.tight_layout()
    
    # 2. 绘制归一化后的数据
    plt.figure(figsize=(5, 3))
    normalized_data = channel_data_np # normalize_by_column_median(channel_data_np)
    ...
    for i in range(channels_to_show):
        ...
        # 对每个通道数据进行下采样
        # 使用 scipy.signal.decimate 进行下采样，它包含抗混叠滤波器
        downsampled_channel_data = signal.decimate(np.array(channels[i]), downsample_factor)
        downsampled_time_stamps = np.arange(len(downsampled_channel_data)) / new_fs

        offset_data = downsampled_channel_data + i * y_offset
        plt.plot(downsampled_time_stamps, offset_data, color="k", linewidth=0.5)
        # 添加通道分隔线
    ...
    # 计算相关矩阵
    corr_matrix = np.corrcoef(channel_data_np)
    ...
    for i in range(channels_to_show):
        # 对原始数据进行下采样以进行频谱分析
        downsampled_channel_data_for_psd = signal.decimate(np.array(channels[i]), downsample_factor)
        f, Pxx = signal.welch(downsampled_channel_data_for_psd, new_fs, nperseg=new_fs*1) # 使用新的采样率 new_fs
        
        # 限制频率范围到 0-100Hz
        mask = f <= new_fs/2 # 使用 new_fs 进行掩码
...
if __name__ == "__main__":
    ...
    if not os.path.exists(file_path):
        ...
        # 可视化数据
        visualize_lfp_data(data, 20000) # 假设输入信号的原始采样率是20kHz