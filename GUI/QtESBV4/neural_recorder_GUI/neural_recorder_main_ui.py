...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.rf_serial_combo.addItem("选择RF串口")
        status_group_layout.addWidget(self.rf_serial_combo)
        
        # RF串口刷新按钮
        self.rf_refresh_button = QPushButton("刷新")
        ...
        # RF串口连接按钮
        self.rf_connect_button = QPushButton("RF连接")
        ...
        # RF power开启按钮
        self.rf_power_on_button = QPushButton("RF开启")
        ...
        # RF power关闭按钮
        self.rf_power_off_button = QPushButton("RF关闭")
    ...
    def refresh_rf_serial_ports(self):
        ...
        try:
            ...
            if ports:
                for port in ports:
                    self.rf_serial_combo.addItem(port)
            else:
                self.rf_serial_combo.addItem("无可用串口")
        except ImportError:
    ...
    def connect_rf_serial(self):
        ...
        if selected_port == "选择RF串口" or selected_port == "无可用串口":
            QMessageBox.warning(self, "Warning", "请先选择一个有效的RF串口")
            return
        ...
        try:
            ...
            self.rf_connect_button.setText("断开RF")
            QMessageBox.information(self, "Success", f"RF控制串口 {selected_port} 连接成功")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"RF控制串口连接失败: {str(e)}")
            self.rf_connected = False
    ...
    def disconnect_rf_serial(self):
        ...
        self.rf_connect_button.setText("连接RF")
        QMessageBox.information(self, "Info", "RF控制串口已断开")
