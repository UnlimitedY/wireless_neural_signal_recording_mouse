...
void loop()
{
  ...
  {                              // receiving data from PC
    ...
    case 'A': // read All info
      ...
      sprintf(outBuffer, "A%d;%d;%d;%d;%d;%d;%.2f;%lu;%d;", S.reward_left, S.reward_right, S.reward_middle, S.low_light_intensity, S.high_light_intensity, S.currProtocolIndex, 20+random(0,100)/100.0, Teensy3Clock.get(), S.extra_TimeOut);
      Serial.println(outBuffer);
      break;
...
void construct_matrix_and_Run()
{
   ...
   *  TrialStart -> PreCueDelayPeriod(200 ms) -> SamplePeriod(500 ms) -> DelayPeriod(~500 ms) -> Cue(100 ms) -> ResponsePeriod -> TrialEnd
