...
byte smartFlag[4] = {0, 0, 0, 0}; //smartFlag for Loop, smartFlag[0] is Noise flag, other flags will be defined in future
...
void gpSMART_Runner() {
  if (smartRunning) {
    ...
    for (int x = 0; x < nLicks; x++) {
      PortLickLineValue[x] =  digitalRead(gpSMART_DI_Lines[x]); // (currtouched_8 >> x) & 1;
    }
    for (int x = 0; x < nLicks; x++) {
      // Determine port entry events
...
void setStateOutputs(byte iState) {
  ...
  if (sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + 1] > 0) {
    ...
    if(sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + 1] == 2){
      Serial.write(sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + 1]); 
  ...
  // Column 25-28: smartFlag Output 1-4; Value: 0 or 1
  for (int x = 0; x < nFlags; x++) {
    smartFlag[x] = sma.OutputMatrix[iState][x + nDOs + nPWMs + ntPWMs + 5];
  }

  // Column 29: DAC1 output: sine waveform; amp is fixed to 1V; frequency: 1000Hz
