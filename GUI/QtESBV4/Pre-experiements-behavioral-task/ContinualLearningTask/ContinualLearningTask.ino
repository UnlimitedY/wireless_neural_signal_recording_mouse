...
const int Frame = 24;    // frame rate: 24 frames per second
  ...
  {                              // receiving data from PC
    ...
    if (smartFlag[1] == 1) // Inactive output
      ...
      // if the state machine is not running , then align the time between HABITS and wireless recorder
        // open all leds for 1/frames sec
        analogWrite(5,10);
        analogWrite(9,10);
        analogWrite(20,10);
        delay(1000/Frame);
...
void construct_matrix_and_Run()
{
  ...
  // trigger-fixation-period（100ms -> 500ms）+ sample period (500ms fixation) + delay period(200ms) + Cue(100ms): 5 steps (100% free reward, [Earlylick < 20 advanced])
  case 0: // 100 ms
  case 1: // 300 ms
  case 2: // 500 ms
  case 3: // sample period
  case 4: // delay period
  {   
    int SampleCueEL = 0; int DelayEL = 0;      
    if(S.currProtocolIndex == 3){
      SampleCueEL = 1;
    } 
    else if(S.currProtocolIndex == 4){
      DelayEL = 1;
      SampleCueEL = 1;
    }
    ...
    if(TrialBlockOnset){// trial block onset
      ...
      TrialBlockOnsetEL = "EarlyLickAborted";
    }
    
    StateTransition TrialBlockDelay_Cond[2] = {{"Tup", TrialBeignState}, {"DI1Rising" ,TrialBlockOnsetEL}}; 
    StateTransition NeuralReader_Cond[2] = {{"Tup", "CapReinit"}, {"DI1Rising" ,"EarlyLickAborted"}}; 
    StateTransition CapReinit_Cond[2] = {{"Tup", "TrialStart"}, {"DI1Rising" ,"EarlyLickAborted"}}; 
    StateTransition TrialStart_Cond[1] = {{"Tup", "PreCueDelayPeriod"}}; 
    StateTransition PreCueDelayPeriod_Cond[2] = {{"Tup", "SampleCue"} ,{"DI1Rising", "EarlyLickAborted"}};
    StateTransition EarlyLickAborted_Cond[1] = {{"Tup" , "TimeOutEL"}};
    StateTransition DelayPeriod_Cond[2]   = {{"Tup", ActionAfterDelay} ,{"DI1Rising", "EarlyLickAborted"}};
    ...
    StateTransition SampleCue_Cond[2]   = {{"Tup","DelayPeriod" }, {"DI1Rising", "EarlyLickAborted"}};
    ...
    OutputAction TrialBlockDelay_Output[3] = {GreenLightOutputBackground, LeftLightOutput, RightLightOutput};
    ...
    gpSMART_State states[20] = {};
    // visual flash states and conditions
    states[0] = smart.CreateState("TrialBlockDelay",                 S.TrialBlockInitPeriod,    2,                  TrialBlockDelay_Cond,         3,           TrialBlockDelay_Output); // remove trial initiation without engagement
    states[18] = smart.CreateState("NeuralReader",                   100,                       2,                  NeuralReader_Cond,            1,           NeuralReader_Output); 
    states[19] = smart.CreateState("CapReinit",                      100,                       2,                  CapReinit_Cond,               1,           CapReinit_Output); 
    ...
    states[3] = smart.CreateState("PreCueDelayPeriod",               100,                       2,                  PreCueDelayPeriod_Cond,       0,           NoOutput); // for islating the noised alignment signal
    states[11] = smart.CreateState("EarlyLickAborted",               100              ,         1,                  EarlyLickAborted_Cond,        1,           ErrorOutput); 
    states[13] = smart.CreateState("SampleCue",                      S.SamplePeriod,            1 + SampleCueEL   , SampleCue_Cond,               1,           Sample_Output); // 500ms
    states[1] = smart.CreateState("DelayPeriod",                     S.DelayPeriod,             1 + DelayEL,        DelayPeriod_Cond,             0,           NoOutput);
  ...
  break;
  // Loc MT antibias
  case 5: // trigger-fixation-period（500ms）+ sample period (500ms fixation) + delay period(200ms) + Cue(100ms): no free reward; weak punishment 3000ms
  case 6: // delay period advanced; delay period(200ms -> 1000ms): 5 steps; timeout[3000ms -> 8000ms]
  case 7:
  case 8:
  case 9:
  case 10:
  case 11: // retention period: delay period (800 ~1200ms randomly); random trial type with model fitting
  // Freq MT antibias
  case 12: // freq
  case 13: // freq retention random trial type with model fitting delay period (800 ~1200ms randomly)
  // reversal freq MT antibias
  case 14: // reversal freq
  case 15: // reversal retention random trial type with model fitting delay period (800 ~1200ms randomly)
  {       
    S.DelayPeriod = S.DelayPeriod * random(80, 120) / 100.0;
    ...
    StateTransition PreCueDelayPeriod_Cond[2] = {{"Tup", "SampleCue"} ,{"DI1Rising", "EarlyLickAborted"}};
    StateTransition TrialStart_Cond[1] = {{"Tup", "PreCueDelayPeriod"}}; 
    StateTransition DelayPeriod_Cond[2]   = {{"Tup", "Cue"} ,{"DI1Rising", "EarlyLickAborted"}};
    StateTransition TrialEnd_Cond[2] = {{"Tup", "TrialBlockEnd"} ,{"DI1Falling" ,"exit"}};
    ...
    gpSMART_State states[25] = {};
    // visual flash states and conditions
    states[0] = smart.CreateState("TrialBlockDelay",                 S.TrialBlockInitPeriod,    2,                  TrialBlockDelay_Cond,         3,           TrialBlockDelay_Output); // 
    states[23] = smart.CreateState("NeuralReader",                   100,                       2,                  NeuralReader_Cond,            1,           NeuralReader_Output); 
    states[24] = smart.CreateState("CapReinit",                      100,                       2,                  CapReinit_Cond,               1,           CapReinit_Output); 

    states[20] = smart.CreateState("TrialStart",                     6,                         1,                  TrialStart_Cond,              1,           TrialStart_Output); // msec
    states[3] = smart.CreateState("PreCueDelayPeriod",               100,                       2                 , PreCueDelayPeriod_Cond,       0,           NoOutput);
    states[11] = smart.CreateState("EarlyLickAborted",               100              ,         1,                  EarlyLickAborted_Cond,        1,           ErrorOutput); 
    states[16] = smart.CreateState("SampleCue",                      S.SamplePeriod,            2                 , SampleCue_Cond,               1,           Sample_Output); // 500ms
    states[1] = smart.CreateState("DelayPeriod",                     S.DelayPeriod,             2 ,                 DelayPeriod_Cond,             0,           NoOutput);
    ...
    states[15] = smart.CreateState("StopLickingReturn",              100,                       1,                  Tup_StopLicking_Cond,         0,           NoOutput);
...
{ 
 ...
 if (S.currProtocolTrials > 100 && EarlyLick100 < 25)
  //  if (S.currProtocolTrials > 10)
  {                        
    S.currProtocolIndex = 5; // Early training
    S.currProtocolTrials = 0;
    S.currProtocolPerf = 0;
  ...
  case 11: // loc retention
 if (S.currProtocolTrials >= 1000 && Perf100 > 80)
  //  if (S.currProtocolTrials > 10)
  {
    S.currProtocolTrials = 0;
    S.currProtocolPerf = 0;
    S.currProtocolIndex = 12;
    S.rule = 0;  // aud freq
  ...
  case 13: // aud freq retention
   if (S.currProtocolTrials >= 1000 && Perf100 > 80)
    //  if (S.currProtocolTrials > 10)
    {
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
      S.currProtocolIndex = 14;
  ...
  case 14: // aud freq reversal
  if (S.currProtocolTrials >= 500 && int(currProtocolPerf_corrected * 100) >= 75)
// if (S.currProtocolTrials > 10)
    {
      S.rule = 2; //  aud freq reversal
      S.currProtocolIndex = 15; // retention reversal aud freq
      S.currProtocolPerf = 0;
      S.currProtocolTrials = 0;
